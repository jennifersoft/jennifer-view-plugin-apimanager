...
const api_list = [
    ...
    { url : '/api/realtime/business', title : '['+i18n.get('ui.label.realtimeData')+'] ' + i18n.get('ui.label.business'), tableKeyField : 'RealtimeBusinessData', query : [
            { key : 'domain_id', title : i18n.get("ui.label.sid"), type : 'short', required : true , input : { type : 'select', content : 'domain', change : function () {
                        ...
                        aries.extension.api("/business", { domain_id: domain_id }, function(data) {
                            update_business_select($("#business_id"), data, { required : false });
                            $("#business_id").change();
                        });
                    }}},
    ...
    { url : '/api/dbsearch/event', title : '[' + i18n.get('ui.label.analysis') + '] ' + i18n.get('ui.label.search.event'), tableKeyField : 'EventList', query : [
            { key : 'domain_id', title : i18n.get("ui.label.sid"), type : 'short', required : true , input : { type : 'select', content : 'domain', change : function () {
                        ...
                        aries.extension.api("/instance", { domain_id: domain_id }, function(data) {
                            update_instance_select($("#instance_id"), data, { required : false });
                            $("#instance_id").change();
                        });
                        aries.extension.api("/business", { domain_id: domain_id }, function(data) {
                            update_business_select($("#business_id"), data, { required : false });
...
const param_keys = ['key', 'title', 'value', 'defaultValue', 'type', 'required'];

const SORT_BY_METRICS = [
    "calls",
    "failures",
    "badResponses",
...
const SORT_BY_METRICS_FOR_SQL_NAME = [
    i18n.get("ui.mx.service_count"),
    i18n.get("ui.mx.service_err_count"),
    i18n.get("ui.mx.service_slow_count"),
    i18n.get("ui.mx.service_time"),
...
function create_param_input(query, key) {
    ...
    if (!initData) {
        ...
        if (input.type == 'select') {
            ...
            $input = $("<input type='hidden' class='param' name='"+key+"' /><input type='datetime-local' class='date input' name='"+key+"_date' value='"+ (moment().format("YYYY-MM-DDT00:00:00")) +"' />");
...
function create_metrics_select(key, input) {
    ...
    aries.extension.api('/metrics', function(metrics) {
...
function make_param (o) {
    var param = {
        index: o.index,
        url : aries.extension.options.hostName + o.url,
        data : make_data(o)
    };
...
function make_data (o) {
    ...
    for(var i = 0, len = query.length; i < len; i++) {
        ...
        if ($input.attr('type') == 'hidden') {
            ...
            if ($.trim(dateStr)) {
                timestamp =  moment(dateStr, 'YYYY-MM-DDThh:mm').valueOf();
            }
    ...
    data.token = aries.extension.options.apiToken;
    ...
    $.ajax({
        ...
        success : function  (data, res, xhr) {
            if (type == 'xml') {
                ...
                $("#data-format").data('index', param.index);
                $("#data-format pre").html(JSON.stringify(data));
                $("#data-table div").data('done', false);
                convert_table(data);
            }
        },
        error : function (res) {
            if(res.status == 404) {
                alert("You must set 'host_name' and 'api_token' in the administration screen.");
            } else {
...
jui.ready(['ui.tab'], function(tab) {
    ...
    $api.on('change', function () {
        ...
        aries.extension.api('/domain', function(data) {
            set_params({
                domain : data
            });
